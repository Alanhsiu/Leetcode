...
| **Reverse**                | `std::reverse(vec1.begin(), vec1.end());`                                                                                                                         | Reverses the order of elements.                                                 |
| **Swap**                   | `std::swap(vec[i], vec[j]);`                                                                                                                                           | Swaps elements at index `i` and `j`.                                            |
| **Clear All**              | `vec.clear();`                                                                                                                                                   | Removes all elements, size becomes 0.                                           |
| **Iteration**              | `for (int x : vec) { /* use x */ }`                                                                                                                              | Range-based for loop (preferred).                                               |
